...
float BKE_vertarray_size(MVert *mvert, int numVerts, int axis)
{
	int i;
	float min_co, max_co;
...
float BKE_length_fitcurve(ArrayModifierData *amd, struct Scene *scene)
{
	float length = 0;
...
static float rand_max_fl(float max)
{
	return BLI_frand() * max;
}

static void array_scale_offset(const float max_off[3], float rit[3], int prop, int sign, int seed)
{
...
static void array_offset(const float max_off[3], float rit[3], int sign, int seed)
{	
	int j;
...
void BKE_init_offset(const int start, const int end, ArrayModifierData *ar)
{
	int i;
...
void BKE_create_offset(const int n, const int totmat, ArrayModifierData *ar, Object *ob)
{
	float loc[3];
	float rot[3];
	float scale[3];
...
void BKE_reset_offset(float offset[4][4], const int count, float loc[3], float rot[3], float scale[3])
{
	...
	loc[0] = loc[0] + offset[3][0];
	loc[1] = loc[1] + offset[3][1];
	loc[2] = loc[2] + offset[3][2];
	...
	unit_m4(app);
	loc_eul_size_to_mat4(app, loc, rot, scale);
	copy_m4_m4(offset, app);
}
...
void dist_point_nurb(Object *obedit, int number_cuts)
{
	...
	Nurb *nu;
	BezTriple *prevbezt, *bezt, *beztnew, *beztn = NULL;
